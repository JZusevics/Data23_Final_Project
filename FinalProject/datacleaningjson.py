...
def internal_id_append(name_dates, candidate_json, candidate_id):
    for name in list(name_dates):
        candidate_json[name]['internal_id'] = candidate_id
        candidate_id += 1
...
def bool_fix(name_dates, candidate_json, yes_no_key):
    ...
    :param name_dates: The dictionary key we are using
    :param candidate_json: Original json file
...
internal_id_append(candidate_json_keys, candidate_json, candidate_id)
